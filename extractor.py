...
def _append_stats(model_short_name, model, multimodal, total, rows_with_values, rows_empty, field_counts):
    fieldnames = ["datetime", "model_short_name", "model_full_name", "multimodal", "total", "rows_with_values", "rows_empty"] + ALL_FIELDS
    row = {
...
def run_extraction(model_short_name):
    if model_short_name not in VALUE_MODELS:
        ...
        sys.exit(1)
    model = VALUE_MODELS[model_short_name]["model"]
    multimodal = VALUE_MODELS[model_short_name]["multimodal"]
    ...
    with open(IN_FILENAME, "r") as infile, open(out_filename, "w") as outfile:
        ...
        for row_num, row in enumerate(reader):
            ...
            try:
                ...
                fields = _parse_llm_response(response)
    ...
    _append_stats(model_short_name, model, multimodal, total, rows_with_values, rows_empty, field_counts)
